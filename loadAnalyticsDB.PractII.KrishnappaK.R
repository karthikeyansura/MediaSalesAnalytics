...
timeAggregatedFactsETL <- function(analyticsDbCon) {
  cat("Loading time aggregated facts...\n")
  ...
  cat("Daily aggregated facts loaded.\n")
  ...
  cat("Monthly aggregated facts loaded.\n")
  ...
  cat("Quarterly aggregated facts loaded.\n")
  ...
  cat("Yearly aggregated facts loaded.\n")
  
  # Report total count
  aggCount <- dbGetQuery(analyticsDbCon, "SELECT COUNT(*) as count FROM time_agg_facts")
  cat(sprintf("Total of %d time aggregated facts records loaded.\n", aggCount$count))
  printLine()
}
